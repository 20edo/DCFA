...
model_example
cd .. 
[w, U, k] = ROM_solver(30, model.M, model.K, 0.5);
cd Test 
cd Playground 
M = model.M; 
K = model.K; 
C = model.C; 
deg = 4;
f = @(t) [zeros(deg-1,1); 5*1e9*sin(t)*(t>0) ; zeros(size(model.M,1)-deg,1)];
% f = @(t) [zeros(14,1); 1e6*(t>0) ; zeros(6,1); 1e6*(t>0); zeros(size(model.M,1)-22,1)];
tspan = [-1,1000]; 
y0 = zeros(2*size(M,1),1); 
N = 30; %size of the reduced order problem
